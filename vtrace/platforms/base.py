...
class TracePlatform:
    ...
    def _plat_getregctx(self,tid): pass
    def _plat_setregctx(self,tid,ctx): pass
...
class TracerBase(vtrace.Trace):
    ...
    def __init__(self):
        ...
        libinfo = {
            'libaddr':'library base address',
            'libname':'normalized library name',
            'libpath':'raw filesystem path',
        }
...
#
    ...
    def _sync_regs(self):
        ...
        """
        regcache = self.cache.pop('regs',None)
        if regcache == None:
            return

        for tid,ctx in regcache.items():
            if not ctx.isDirty():
                continue
            self._plat_setregctx(tid,ctx)
...
#
    ...
    def _clear_breakpoints(self):
        for addr,info in self.breaks.items():
